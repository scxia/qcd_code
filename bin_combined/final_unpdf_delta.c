...
void main(int argc, char* argv[])
{
	...
	twop_delta(&upropsmear, &dpropsmear, &geo, x_src, t_start, t_stop, mom, fp_corr_p);
	...
	for (is = 0; is < 12; is++)
	{
		mksource_su_delta(&vecsou, &upropsmear, &dpropsmear, &uAPE, &geo, x_src, is, alpha, nsmear, keci, mom, tsink);
		mg_solve_down((double*)vecout.D, (double*)vecsou.D, accur);
		qcd_copyPropagatorVector(&suprop, &vecout, is / 3, is % 3);

		mksource_sd_delta(&vecsou, &upropsmear, &dpropsmear, &uAPE,& geo, x_src, is, alpha, nsmear, keci, mom, tsink);
		mg_solve_up((double*)vecout.D, (double*)vecsou.D, accur);
