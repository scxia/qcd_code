...
void qcd_subAdjoint3x3(qcd_complex_16 sum[3][3], qcd_complex_16 summand1[3][3], qcd_complex_16 summand2[3][3])
{
    qcd_uint_2 c1, c2;
    for (c1 = 0; c1 < 3; c1++)
        for (c2 = 0; c2 < 3; c2++)
            sum[c1][c2] = qcd_CSUB(summand1[c1][c2], qcd_CONJ(summand2[c2][c1]));
    return;
}//end qcd_subAdjoint3x3
...
void qcd_Adjointmul3x3(qcd_complex_16 product[3][3], qcd_complex_16 factor1[3][3], qcd_complex_16 factor2[3][3])
{
	qcd_uint_2 c1, c2, c3;
	memset(&(product[0][0].re), 0, 9 * sizeof(qcd_complex_16));
	for (c1 = 0; c1 < 3; c1++)
	...
	for (c3 = 0; c3 < 3; c3++)
	{
		product[c1][c2] = qcd_CADD(product[c1][c2], qcd_CMUL(qcd_CONJ(factor1[c3][c1]), factor2[c3][c2]));
	}
	return;
}
...
void qcd_zeroGaugeTransformation(qcd_gaugeTransformation* u)
{
    memset(&(u->D[0][0][0].re), 0, 3 * 3 * (u->geo)->lV * sizeof(qcd_complex_16));
}//end qcd_zeroGaugeField
...
void qcd_mulPropagatorC(qcd_propagator *propres,qcd_propagator *prop, qcd_complex_16 c)
{
   qcd_uint_8 i;
   ...
   for(a=0; a<3; a++)
   for(b=0; b<3; b++)
      propres->D[i][mu][nu][a][b] = qcd_CMUL(prop->D[i][mu][nu][a][b],c);
}


void qcd_mulPropagatorC3d(qcd_propagator *prop, qcd_complex_16 c, qcd_int_4 t)
{
   qcd_int_8 i,v;
   qcd_int_2 mu,nu,a,b;
...
void qcd_addPropagator(qcd_propagator *propsum, qcd_propagator *propsum1, qcd_propagator *propsum2)
{
	qcd_int_8 v, mu, nu, c1, c2;
	for (v = 0; v < propsum->geo->lV; v++)
	for (mu = 0; mu < 4; mu++)
	for (nu = 0; nu < 4; nu++)
	for (c1 = 0; c1 < 3; c1++)
	for (c2 = 0; c2 < 3; c2++)
	{
		propsum->D[v][mu][nu][c1][c2] = qcd_CADD(propsum1->D[v][mu][nu][c1][c2], propsum2->D[v][mu][nu][c1][c2]);
	}
}
...
void qcd_projectSU3(qcd_gaugeField *gf)
{
   ...
   */
   ...
   for(mu=0; mu<4; mu++) 
   {
	  ...
	  //这里的H 相当于H^2
      /*
        Assure Hermiticity:  //由于舍入误差，这步相当于对称化
      */
