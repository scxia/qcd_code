...
char* qcd_getParams(char* fname,int *len)
{
   ...
   *(len)=i;      //计算文件内容的长度
   rewind(pfile);
   params = (char*)malloc(*(len)*sizeof(char)); //申请内存

   fread(params,sizeof(char),*len,pfile);  //将硬盘数据写入内存
...
/* by Giannis Koutsou */
char* qcd_getParam(char token[],char* params,int len) //得到token[]所在的位置
{
   ...
   for(i=0;i<len-token_len;i++)
   {
      ...
      {
         i+=token_len;
         *(strchr(params+i,'<'))='\0';        //数据为格式为<数据名> 数据 <数据名>  
		 break;                              //将< 变成结尾，输入的时候做截断
      }
   }
   return params+i;
}
...
int qcd_getGaugeLime(char *fname, qcd_gaugeField *u)
{
   ...
   if(isDouble)
   {
      ...
      if(!qcd_isBigEndian()){
	//if(u->geo->myid == 0) printf("big endian\n");
...
int qcd_getPropagatorLime(char *fname, qcd_propagator *p)   //读12个矢量拼成一个prop
{
   char vecnames[12][qcd_MAX_STRING_LENGTH];
   FILE *fid;
   qcd_int_4 i;
		...
		for(c1=0; c1<3; c1++)
		  {
		    p->D[qcd_LEXIC(t,x,y,z,p->geo->lL)][mu][nu][c1][c2] = 
		      v->D[qcd_LEXIC(t,x,y,z,v->geo->lL)][mu][c1];
		  }
...
int qcd_writePropagatorLime(char *fname, qcd_propagator *p)
{
	char vecnames[12][qcd_MAX_STRING_LENGTH];
	FILE *fid;
	qcd_int_4 i;
	...
	{
		i++;
		fscanf(fid, "%s", &(vecnames[i][0]));     //当输出文件包含不止12个矢量时，这里可能内存出错
	}
	fclose(fid);
	if (i != 12)
	{
	...
	/*fid = fopen(vecnames[0], "w");
	if (fid == NULL)
	{
		fprintf(stderr, "process %i: Error in %s! Cannot open %s for writing\n", p->geo->myid, __func__, vecnames[0]);
		return(1);
	}
	fclose(fid); */
	...
	for (nu = 0; nu < 4; nu++)
		...
		qcd_copyVectorPropagator(v, p, nu, c2);
		qcd_writeVectorLime(vecnames[i], qcd_PROP_LIME, v);
		if (p->geo->myid==0) printf("prop %d/12 is written \n", i);
		}
...
int qcd_writePropagator(char *fname, int type, qcd_propagator *p)
{
   ...
   switch(type)
   {
	  ...
	  case qcd_SOURCE_LIME:
		  return(qcd_writePropagatorLime(fname,p));
		  break;
      default:
